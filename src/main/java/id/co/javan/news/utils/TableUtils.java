package id.co.javan.news.utils;
...
public class TableUtils {
    ...
    public void print() {
        ...
        for (String[] cells : rows) {
            ...
            for (int i = 0; i < cells.length; i++) {
                maxWidths[i] = Math.max(maxWidths[i], (cells[i] != null ? cells[i] : "").length());
            }
        }
    ...
    // st.setShowVerticalLines(true);//if false (default) then no vertical lines are
    // shown
    // st.setHeaders("one", "two", "three");//optional - if not used then there will
    // be no header and horizontal lines
